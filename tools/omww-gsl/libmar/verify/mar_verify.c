 ...
 *
 * @param pathToMARFile The path of the MAR file to verify.
 * @param keyNames     Pointer to the first element in an array of keys names.
 *                      Used only if compiled as NSS, specifies the kyes names
 * @param keyCount     The number of elements in keyNames
 * @return 0 on success
 *         a negative number if there was an error
 *         a positive number if the signature does not verify
 */
int
mar_verify_signatures_from_files(const char *pathToMARFile,
  ...
  for (k = 0; k < keyCount; k++) {
    if (CryptoX_Failed(CryptoX_LoadPublicKeyFromFile(provider, NULL, NULL,
                                             &keys[k], keyNames[k], &certs[k]))) {
      fclose(fp);
      fprintf(stderr, "ERROR: Could not load public key.\n");
      return CryptoX_Error;
    }
  ...
  rv = mar_extract_and_verify_signatures_fp(fp, provider, keys, keyCount);
  fclose(fp);
