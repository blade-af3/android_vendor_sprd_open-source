...
static int write_txt(char * pathname ,int n);
...
static int write_txt(char * pathname ,int n)
{
	...
	//LOGD("mmitest I am here3");
	//LOGD("mmitest name=%s: id=0x%x  yes_no=%d  status=%d \n",test_result_txt[14].name,test_result_txt[14].id,test_result_txt[14].yes_no,test_result_txt[14].status);
	//if(strcmp(pathname,PHONETXTPATH))
...
multi_test_info multi_test_item[MULTI_TEST_CNT] = {
	...
	{21,23,A_PHONE_TEST_WIFI, test_wifi_pretest},
	{20,22,A_PHONE_TEST_BT, test_bt_pretest},
	{22,24,A_PHONE_TEST_GPS, test_gps_pretest}
};
		...
		if(i>=2&&i<=3)
		...
		else if(i>=6&&i<=7)
			{
				...
				if(i==7)
					...
					memset(tmp, 0, sizeof(tmp));
					sprintf(tmp, "%s:%s",MENU_TEST_BCAMERA_RESULT, rl_str2);
					row = ui_show_text(row, 0, tmp);
			}
		}
		else if(i>=8&&i<=9)
		...
		else if(i>=17&&i<=19)
			{
				...
				if(i==19)
					...
					if((strcmp(rl_str, ptrfail) == 0)||(strcmp(rl_str1, ptrfail) == 0)||(strcmp(rl_str2, ptrfail) == 0))
						ui_set_color(CL_RED);
					else if((strcmp(rl_str, ptrna) == 0)||(strcmp(rl_str1, ptrna) == 0)||(strcmp(rl_str2, ptrna) == 0))
						ui_set_color(CL_WHITE);
					else
						ui_set_color(CL_GREEN);
					 memset(tmp, 0, sizeof(tmp));
					 sprintf(tmp, "%s:G:%sL:%sM:%s",MENU_TEST_SENSOR_RESULT, rl_str,rl_str1,rl_str2);
					 row = ui_show_text(row, 0, tmp);
			}
		}
