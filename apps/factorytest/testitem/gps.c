...
int gpsOpen( void )
{
	...
	if(err < 0)
	{
		SPRD_DBG("%s system glgps config error =%d \n",__FUNCTION__,err);
		return -1;
	}

	err = hw_get_module(GPS_HARDWARE_MODULE_ID, (hw_module_t const**)&module);
	if (err == 0) {
		SPRD_DBG("%s call hw_get_module() success \n",__FUNCTION__);

		err = module->methods->open(module, GPS_HARDWARE_MODULE_ID,(hw_device_t**)&device);
		if (err == 0) {
			SPRD_DBG("%scall open() GPS MODULE success \n",__FUNCTION__);
			sGpsIntrfc = device->get_gps_interface(device);
		}
	}

	if (sGpsIntrfc)
		SPRD_DBG("%scall get_gps_interface() success \n",__FUNCTION__);
	else
	{
		SPRD_DBG("%scall get_gps_interface() failed \n",__FUNCTION__);
		return -1;
	}
	...
	{
		SPRD_DBG("sGpsIntrfc->init(&sGpsCallbacks) failed \n");
		return -1;
	}
	ret = sGpsIntrfc->start();
	if (ret != 0)
	{
		SPRD_DBG("sGpsIntrfc->start() failed ret = %d \n", ret);
	}
	else
		SPRD_DBG("sGpsIntrfc->start()  %s \n", "success");

	gps_is_enable = 1;
	gOpenTime = time(NULL);
	SPRD_DBG("gOpenTime=%ld \n", gOpenTime);
	sSVNum = 0;
	FUN_EXIT;
	return 0;
}
...
static void * processThread_show( void * param )
{
	...
	while(thread_run == 1)
	{
		...
		{
			gps_show_result(1);
			SPRD_DBG("GPS Test PASS Catch the Star\n");
