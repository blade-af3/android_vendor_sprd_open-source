...
int eng_wpa_scan()
{
	...
	system("mkdir /data/misc/wifi/sockets");
	system("chown wifi.wifi /data/misc/wifi/sockets");
	system("chmod 0777 /data/misc/wifi/sockets");
	...
	if(access("/data/misc/wifi/fwpath",F_OK)!=0 ||
		...
		SPRD_DBG("Wifi cfg file err\n");
		return -1;
	}
	system("rmmod ittiam");
	system("insmod /system/lib/modules/ittiam.ko");
	system("wpa_supplicant -iwlan0 -Dnl80211 -c/data/misc/wifi/wpa_supplicant.conf -dd &");
	sleep(2);
	while(count){
		if(system("wpa_cli -iwlan0 -p/data/misc/wifi/sockets scan")<0)
		{
			SPRD_ERR("MMI wifi error: wpa_cli scan!\n");
			return -1;
		}
		sleep(1);
	...
	if(count<=0)
	{
		SPRD_ERR("MMI wifi error: no result!\n");
		return -1;
	}
	return 0;
}
...
int eng_wifi_scan_start(void)
{
	...
	while(1)
	{
		...
		{
			SPRD_ERR("MMI wifi error: wpa_cli terminate!\n");
		}
		usleep(100);
	#endif
	...
	if(system("rmmod ittiam"))
	{
		SPRD_ERR("MMI wifi error: rmmod ittiam!\n");
	}
