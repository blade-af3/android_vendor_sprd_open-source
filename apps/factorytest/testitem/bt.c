 ...
 *       Filename:  bt.c
 *
 *    Description:  Bluetooth IMM Test API
 *
 *        Version:  1.0
 *        Created:  2014-09-15/13:24:38
 *         Author:  zailin.zhang,haiyan.liu
 *   Organization:  Spreadtrum
 *
 * =====================================================================================
 */
...
void check_return_status(bt_status_t status)
{
    ...
    {
        SPRD_DBG("HAL REQUEST FAILED status : %d (%s)", status, dump_bt_status(status));
    }
    else
    {
        SPRD_DBG("HAL REQUEST SUCCESS");
    }
}
...
void create_mac_rand(char *btmac)
{
    ...
    //FOR BT
    i=rand(); j=rand();
    //SPRD_DBG("%s:  rand i=0x%x, j=0x%x",__FUNCTION__, i,j);
...
int read_local_bt_mac(const char * file_path,  char * mac)
{
    ...
    {
        SPRD_DBG("the mac file %s: %s is not exists",__FUNCTION__, BT_MAC_FILE_PATH);
    ...
    if(fd_btaddr>=0)
    {
       read(fd_btaddr, bt_mac,BT_MAC_STR_LEN);

       bt_mac[BT_MAC_STR_LEN] = '\0';

       memcpy(mac,bt_mac,sizeof(bt_mac));
...
{
    SPRD_DBG("ADAPTER STATE UPDATED : %s", (state == BT_STATE_OFF)?"OFF":"ON");

    if (state == BT_STATE_ON)
    {

     SPRD_DBG(":::::::::::::::state = BT_STATE_ON:::::::::::::::::::::::::::::::::");
...
void discovery_state_changed_cb(bt_discovery_state_t state)
{
    switch(state)
    {
        case BT_DISCOVERY_STOPPED:
            ...
            SPRD_DBG("BT Discovery State Changed: BT_DISCOVERY_STOPPED");
        ...
        case BT_DISCOVERY_STARTED:
            ...
            SPRD_DBG("BT Discovery State Changed: BT_DISCOVERY_STARTED");

            break;

        default:
            SPRD_DBG("BT Discovery State Changed: BT_DISCOVERY_ERROR");
    }
}
...
static int eng_bt_hal_load(void)
{
    ...
    SPRD_DBG("Loading HAL lib + extensions");
    ...
    SPRD_DBG("HAL library loaded (%s)", strerror(err));
...
void eng_bt_start_discovery(void)
{
    if (!bt_enabled)
    {
        SPRD_DBG("Bluetooth must be enabled for bt scan to work.");
        return;
    }
...
int eng_bt_open( void )
{
    ...
    {
        SPRD_DBG("BT MMI:HAL failed to initialize, exit\n");
    }
...
static void config_permissions(void)
{
    ...
    SPRD_DBG("set_aid_and_cap : pid %d, uid %d gid %d", getpid(), getuid(), getgid());
...
void eng_bt_mac (void)
{
    char bt_mac_address[64] = "";
	ui_set_color(CL_WHITE);
	ui_show_text(2, 0, TEXT_BT_SCANING);
    gr_flip();

    memset(local_bd_str,0,sizeof(local_bd_str));

    if(read_local_bt_mac(BT_MAC_FILE_PATH,local_bd_str)== 1) {
        SPRD_DBG("Local MAC address:%s",local_bd_str);
    } else {
        create_mac_rand((char*)local_bd_str);
        SPRD_DBG("Randomly generated MAC address:%s",local_bd_str);
    }

    SPRD_DBG(" bt_mac_address addr = %s", bt_mac_address);
    //sprintf(bt_mac_address,"%s:%s", TEXT_BT_MAC, local_bd_str);
	ui_clear_rows(2, 1);
	ui_set_color(CL_WHITE);
	ui_show_text(2, 0, TEXT_BT_MAC);
	ui_show_text(3, 0, local_bd_str);
    gr_flip();
}
