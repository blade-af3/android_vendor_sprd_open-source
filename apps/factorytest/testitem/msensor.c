...
static int do_read(void)
{
	...
	LOGD("mmitest here tpye_num=%d\n",type_num);
	/*********activate_sensors(ON)***************/
	for (i = 0; i < count; i++) {
		err = activate_sensors(i, 1, S_ON);
		if (err != 0) {
			LOGD("mmitest activate_sensors(ON) for '%s'failed \n",list[i].name);
			return 0;
		}
	}
	do {
		...
		//LOGD("mmitest read %d events:\n", n);
		for (i = 0; i < n; i++) {
			data = buffer[i];

			if (data.version != sizeof(sensors_event_t)) {
				    LOGD("mmitest incorrect event version (version=%d, expected=%d",data.version, sizeof(sensors_event_t));
				break;
			}
...
funend:
	...
	if (err != 0) {
		LOGD("mmitest activate_sensors(OFF) for '%s'failed\n",list[i].name);
		return 0;
		}
	}
	err = sensors_close(device);
	if (err != 0) {
		LOGD("mmitest sensors_close() failed \n");
	}
	return err;	
}
...
static void msensor_thread(void *param)
{
	...
	LOGD("mmitest here m1");
    err=hw_get_module(SENSORS_HARDWARE_MODULE_ID,(hw_module_t const **)&module);
	if (err!= 0)
	{
		LOGD("mmitest hw_get_module() failed\n");
		return ;
	}

	err = sensors_open(&module->common, &device);
	if (err != 0) {
		LOGD("mmitest sensors_open() failed\n");
		return ;
	}
	count = module->get_sensors_list(module, &list);

	ptr=list[1].name;
	LOGD("mmitest here m2 %s",ptr);

	ui_set_color(CL_WHITE);
	cur_row = ui_show_text(cur_row, 0, TEXT_MS_DEV_INFO);
