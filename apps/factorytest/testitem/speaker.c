...
void* playback_thread(void* t_mode){
	...
	if(mode->mode == SPEAKER){
		sprintf(cmd+strlen(cmd),"tinymix -D %d 63 1;",card_num);
		sprintf(cmd+strlen(cmd),"tinymix -D %d 64 1;",card_num);
		sprintf(cmd+strlen(cmd),"tinymix -D %d 87 1;",card_num);
		...
		sprintf(cmd+strlen(cmd),"tinymix -D %d 89 1;",card_num);
		sprintf(cmd+strlen(cmd), "%s %s -D %d","tinyplay", SPRD_AUDIO_FILE,card_num);
        LOGD("mmitestsound cmd=%s",cmd);
	}
	...
	if(mode->mode == SPEAKER){
		sprintf(cmd+strlen(cmd),"tinymix -D %d 63 0;",card_num);
		sprintf(cmd+strlen(cmd),"tinymix -D %d 64 0;",card_num);
		sprintf(cmd+strlen(cmd),"tinymix -D %d 87 0;",card_num);
	}	
	else if(mode->mode == FM_PLAY){
		...
		sprintf(cmd+strlen(cmd),"tinymix -D %d 89 0;",card_num);
	}
	system(cmd);
err:
	if(cmd) free(cmd);
